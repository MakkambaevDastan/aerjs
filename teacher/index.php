...
if (!$_SESSION['AVN_User']['schedule']) {
    $id_teacher = $_SESSION['AVN_User']['id_teacher'];
    $s = $w_s == 1 ? "=" : "<>";
    $sql ="SELECT DISTINCT dbo.V_Raspisanie_new.ID,
        ...
        WHERE (dbo.V_Raspisanie_new.id_a_year = $rate_year)
            AND (dbo.V_Raspisanie_new.id_teacher = $id_teacher)
            AND CAST([avn].[dbo].[educ_sh].[id_semester] AS INT) % 2 $s 0
    ...
    $_SESSION['AVN_User']['schedule'] = $schedule;
} else {
    $schedule = $_SESSION['AVN_User']['schedule'];
}
foreach ($time_start as $key => $value) {
    if ($date->format('H.i') > $value && $time_end[$key] > $date->format('H.i')) {
        $now = $key;
    }
}
        ...
        if ($j == 0 and $i == 0) {
            ...
            for ($it = 0; $it < count($schedule[$j][$i]); $it++) {
                if ($schedule[$j][$i][$it]['id_time'] < $now) {
                    ...
                    <h5 class='h5'>" . $schedule[$j][$i][$it]['p34'] . " " .
                        $schedule[$j][$i][$it]['StudyTypeName'] . "</h5>" .
                        $schedule[$j][$i][$it]['number'] . " " .
                        $schedule[$j][$i][$it]['p20'] . "</a>";
                }
                if ($schedule[$j][$i][$it]['id_time'] == $now) {
                    $_SESSION['AVN_User']['now'][$schedule[$j][$i][$it]['id_group']] = $schedule[$j][$i][$it];
                    $_SESSION['AVN_User']['now']['id_time'] = $now;
                    echo "<a class='list-group-item shadow-lg list-group-item-primary list-group-item-action m-1 rounded-lg' href = 'journal_add.php'>
                    <h5 class='h5'>" . $schedule[$j][$i][$it]['p34'] . " " .
                        $schedule[$j][$i][$it]['StudyTypeName'] . "</h5>" .
                        $schedule[$j][$i][$it]['number'] . " " .
                        $schedule[$j][$i][$it]['p20'] . "</a>";
            ...
            if ($i == $now && $j == $day_of_week || $j == $day_of_week) {
                echo "<td class='border table-info '></td>";
            } else {
