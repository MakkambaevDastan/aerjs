...
$group = $_SESSION['AVN_User']['group'][$_GET['id_group']];
if ($group) {
    foreach ($group as $key => $value) {
        $journal[(int)$key] = $_SESSION['AVN_User']['discipline'][$key];
        $journal[(int)$key]['id_group'] = $value['id_group'];
        $journal[(int)$key]['p20'] = $value['p20'];
        $journal[(int)$key]['o_z'] = $value['o_z'];
        $journal[(int)$key]['p21'] = $value['p21'];
        $journal[(int)$key]['id_sub_group'] = $value['id_sub_group'];
        $id_rate = $_SESSION['AVN_User']['discipline'][(int)$key]['id_rate'];
        $p20 = $value['p20'];
        $id_sillabus_t =  $_SESSION['AVN_User']['discipline'][(int)$key]['id_sillabus_t'];
        if ($id_sillabus_t) {
        ...
        WHERE dbo.syllabus_subject.id_sillabus_t = " . $id_sillabus_t . "
            AND ((dbo.syllabus_subject_hour.id_group = " . $value['id_group'] . ")
            ...
            while ($row = sqlsrv_fetch_array($select)) {
                $journal[$key]['subject'][$row['subject_type']][$row['subject_num']] =
                    [
    ...
    WHERE (dbo.syllabus_rating.id_group = " . $value['id_group'] . ")
        AND dbo.syllabus_rating.id_sillabus_t = " . $id_sillabus_t;
            $select_rating = sqlsrv_query($connect_local, $sql_rating);
            while ($row = sqlsrv_fetch_array($select_rating)) {
                $journal[$key]['student'][$row['id_student']][$row['subject_type']][$row['subject_num']] =
                    [
    ...
    $sql = "SELECT DISTINCT [Student].[id_student],
                [Student].[p1],
                [Student].[p2],
                [Student].[p3]
                FROM [dbo].[movement_t]
            LEFT JOIN [dbo].[Student] ON [Student].[id_student] = [movement_t].[id_student]
            WHERE [id_group]= " . $_GET['id_group'] . " AND id_rate = " . $id_rate . " and isStudying <> 0
            ORDER BY [Student].[p1]";
