...
$id_teacher = $_SESSION['AVN_User']['id_teacher'];
...
FROM (
    ...
    WHERE ([avn].dbo.V_Raspisanie_new.id_teacher = $id_teacher)
...
if (count($discipline) > 0) {
    $_SESSION['AVN_User']['discipline'] = $discipline;
    echo "<h1 class='text-center h1'>Дисциплины</h1>";
    if ($_SESSION['message']) {
        echo "<h3 class='text-center h1'>" . $_SESSION['message'] . "</h3>";
        unset($_SESSION['message']);
    }
    echo "<ul class='list-group'>";
    foreach ($discipline as $key => $value) {
        if ($value['confirmation'] === 1) echo "<a class=\"list-group-item list-group-item-success list-group-item-action\" href = 'syllabus.php?id_educ_sh=" . $value['id_educ_sh'] . "'>
            <div class='d-flex w-100 justify-content-between'> <h5 class='mb-1'><small class='text-muted'>Дисциплина: </small>" .
            $value['p34'] . "</h5><small class='text-muted'>" .
            $value['id_semester'] . "-семестр Подтвержден</small></div>
            <p class='mb-1'><small class='text-muted'>специальность: </small>" .
            $value['p25-2'] . "</p></a>";
        else if ($value['confirmation'] === 0) echo "<a class=\"list-group-item list-group-item-danger list-group-item-action\" href = 'syllabus.php?id_educ_sh=" . $value['id_educ_sh'] . "'>
            <div class='d-flex w-100 justify-content-between'> <h5 class='mb-1'><small class='text-muted'>Дисциплина: </small>" .
            $value['p34'] . "</h5><small class='text-muted'>" .
            $value['id_semester'] . "-семестр Не подтвержден</small></div>
            <p class='mb-1'><small class='text-muted'>специальность: </small>" .
            $value['p25-2'] . "</p></a>";
        else echo "<a class=\"list-group-item  list-group-item-action\" href = 'syllabus.php?id_educ_sh=" . $value['id_educ_sh'] . "'>
            <div class='d-flex w-100 justify-content-between'> <h5 class='mb-1'><small class='text-muted'>Дисциплина: </small>" .
            $value['p34'] . "</h5><small class='text-muted'>" .
            $value['id_semester'] . "-семестр Нет силлабус</small></div>
            <p class='mb-1'><small class='text-muted'>специальность: </small>" .
            $value['p25-2'] . "</p></a>";
    }
    echo "</ul>";
